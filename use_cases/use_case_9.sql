...
CREATE INDEX idx_case_desc ON cases(case_desc);
...
EXPLAIN SELECT c.case_ref_id, p.property_id, c.case_desc, c.user_id, c.staff_id, c.date_opened, c.staff_comment
FROM property p
    INNER JOIN cases c ON p.property_id = c.property_id
WHERE
    c.case_desc IN (
...
DROP INDEX idx_case_desc ON cases;